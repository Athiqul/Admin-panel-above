                    ...
                    <?php if($response->errors==false):?>
                      <table class="table">
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td>
                              <a class="text-light text-decoration-none" href="<?=site_url('/admin/show-package/'.$item->packages->id)?>">
                                <?=$item->packages->title?></a
                              >
                            </td>
                            <td><?=$item->packages->category?></td>
                            
                            <td><?=date('j M Y',strtotime($item->packages->created_at))?></td>
                       

                            <td>
                              <?=form_open('/admin/package-update/'.$item->packages->id)?>
                              <input type="hidden" name="status" value="<?=($item->packages->status==1)?"0":"1"?>">
                              <button onclick="return confirm('Are you Sure?')" class="btn <?=($item->packages->status=="1")?"btn-outline-danger":"btn-outline-success"?> " type="sub
                              ">
                                <?=($item->packages->status=="1")?"Hidden":"Activate"?>
                              </button>
                          </form>
                            </td>
