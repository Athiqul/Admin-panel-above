                    ...
                    <?php if($response->errors==false):?>
                      <table class="table">
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td>
                              <a class="text-light text-decoration-none" href="<?=site_url('/admin/show-product/'.$item->id)?>">
                                <?=$item->title?></a
                              >
                            </td>
                            <td><?=$item->brand?></td>
                            ...
                            <td>
                              <?=form_open('/admin/update-product/'.$item->id)?>
                              <input type="hidden" name="stock_status" value="<?=($item->stock_status==1)?"0":"1"?>">
                              <button onclick="return confirm('Are you Sure?')" class="btn <?=($item->stock_status=="1")?"btn-outline-danger":"btn-outline-success"?> " type="sub
                              ">
                                <?=($item->stock_status=="1")?"Stock Out":"Stock In"?>
                              </button>
                          </form>
                            </td>
